...
def URL_link_extractor(url: str) -> set():          # scrape the webSite (url) to extract all the available links
    ...
    for element in matches:
        ...
        try:
            if not valid(element):               # check the validity of the link
                continue
...
if __name__ == "__main__":                             # main class to do the process
        ...
        if CrawlingActivation == ("Y"):                            # crawling activated
            for each in URLlist:
                try:
                    ...
                    print("the URL you entered is not correct", each)
    ...
    if Specifier == '2':                                       # input type for an html web page
        ...
        try:
            ...
            print(red + "you may not copy the " + blue + htmlFile + red + " file into the script directory, or you entered the wrong name")
